...
if($action == "saveChanges"){
	...
	die("Grabando... <script>window.open('tickets_adm.php?msg=Cambios Grabados&message=Cambios Grabados','_self');</script>");
...
if($action == "changeStatus"){
	...
	$res_status = mysql_query($val_status);
	$row = mysql_fetch_row($res_status);
	...
	die("Grabando... <script>window.open('tickets_adm.php','_self');</script>");
...
if($action == "saveNotes"){
	...
	$res = mysql_query($query);
	 ...
	 //die(var_dump($_POST));
	 die("Grabando... <script>window.open('tickets_adm.php?msg=Nota Grabada&message=Nota Grabada&openModal=$id".($filterDepto ? "&action=filterDepto&value=".$filterDepto : "")."','_self');</script>");
	
}

if($action == "saveDepto"){
	// var_dump($_GET);
	// $query = "UPDATE tickets_adm_deptos SET `status` = '$status' WHERE id='$id' LIMIT 1 ";
	...
	die("Grabando... <script>window.open('tickets_adm.php?msg=Departamento Creado Exitosamente&message=Departamento Creado Exitosamente','_self');</script>");
...
$modalContent = "<div style='display:inline-block;width: 45%;'>
					<form action='tickets_adm.php' method='POST' enctype='multipart/form-data' id='modalForm".$obj["id"]."' style='text-align:left;' >
...
if($obj["id"]){
	...
	$modalContent .= "<div style='display:inline-block;width: 45%;vertical-align:top;border-left:1px solid #cfcfcf;padding-left:15px;'>
						<form action='tickets_adm.php' method='POST' enctype='multipart/form-data' id='' style='text-align:left;' >
						<h6>Notas:</h6>
...
$modalContent = "<form action='tickets_adm.php' method='POST' enctype='multipart/form-data' id='deptoForm' style='text-align:left;' >
...
function elimina_depto(id){
	...
	window.open("tickets_adm.php?action=eliminaDepto&id="+id,"_self");
...
function filterDepto(value){
	// console.log(value);
	window.open("tickets_adm.php?action=filterDepto&value="+value,"_self");
	
}

function filterPers(value){
	// console.log(value);
	window.open("tickets_adm.php?action=filterPers&value="+value,"_self");
...
<div class="newButton">
	...
	<?php if($esEncargado){ ?>
		...
		<a href="tickets_adm_historial.php" ><button type="button" class="btn btn-primary btn-sm">Historial</button></a>
